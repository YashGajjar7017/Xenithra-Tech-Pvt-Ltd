...
const Sidebar = ({ collapsed, sidebarWidth, activeActivity }) => {
  ...
  // Read installed and store extensions on load
  useEffect(() => {
    ...
    if (window.api && typeof window.api.getStoreExtensions === 'function') {
      window.api.getStoreExtensions().then((exts) => {
  ...
  const handleUploadExtension = async () => {
    if (window.api && typeof window.api.uploadStoreExtension === 'function') {
      const res = await window.api.uploadStoreExtension()
      if (res && res.success) {
        setStoreExtensions(res.extensions || [])
        alert(res.message)
  ...
  const getGitBadge = (filename) => {
    if (filename === 'compiler.js') {
      return (
        <span style={styles.gitBadgeUntracked} title="Untracked">
          U
        </span>
      )
    }
    if (
      ...
      filename === 'index.js' ||
      filename === 'Topbar.jsx' ||
      filename === 'EditorPage.jsx'
    ) {
      return (
        <span style={styles.gitBadgeModified} title="Modified">
          M
        </span>
      )
    }
    ...
    >
      {activeActivity === 'search' ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            >
              <div style={{ fontWeight: '600', marginBottom: '4px' }}>
                Deep Link Protocol Handover
              </div>
              <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                Registered URI Scheme: <code>xenithra://token:</code>
              </div>
              <div style={{ fontSize: '10px', color: '#00ffaa', marginTop: '4px' }}>
                ✓ Active & Listening for Web Redirects
              </div>
            </div>
            <div
              style={{
            ...
            >
              ...
              <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                Compiler Engine Port: {localStorage.getItem('api-port') || '8000'}
              </div>
              <div
                style={{
                  fontSize: '11px',
              ...
              >
                Active Theme: {(localStorage.getItem('theme') || 'vscode-dark').replace('-', ' ')}
              </div>
            </div>
            <div
              style={{
                    ...
                    <button
                      onClick={() => {
                        window.dispatchEvent(
                          new CustomEvent('open-file', {
                            detail: {
                              filename: `template_${snip.lang}.${snip.lang}`,
                              code: snip.code
                            }
                          })
                        )
        ...
        <React.Fragment>
          ...
          {/* Project Files */}
          <div style={styles.treeContainer}>{renderTree(loadedFolder.tree)}</div>
        </React.Fragment>
      ) : (
        ...
        <div style={{ ...styles.welcomePane, borderStyle: isDragOver ? 'solid' : 'dashed' }}>
          ...
          <p style={styles.welcomeSub}>
            Open a folder or drag & drop directory files here to mount workspace.
          </p>
          ...
          <div style={styles.btnGroup}>
            ...
            <button
              style={{ ...styles.welcomeBtn, background: 'rgba(255,255,255,0.03)' }}
