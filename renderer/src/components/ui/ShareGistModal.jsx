...
const ShareGistModal = ({ isOpen, onClose, filename = 'script.js', code = '' }) => {
  ...
  const handleCreateGist = async () => {
    ...
    if (window.api && typeof window.api.shareGitHubGist === 'function') {
      try {
        const res = await window.api.shareGitHubGist(
          filename,
          code,
          description,
          isPublic,
      ...
      >
        ...
        <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div>
            ...
            >
              GitHub Token (Optional for User Account Gists)
            </label>
            <input
              type="password"
              value={githubToken}
          ...
          {resultUrl && (
            ...
            >
              <div style={{ fontSize: '11px', color: '#00ffaa', fontWeight: 'bold' }}>
                ✓ Gist Published Successfully!
              </div>
              <div style={{ fontSize: '10px', color: '#58a6ff', wordBreak: 'break-all' }}>
                {resultUrl}
              </div>
              <button
